...
public class ChatClientService {
    ...
    public ChatClientService(ChatModel chatModel, ChatClient.Builder chatClientBuilder,
            SpringAiPlaygroundOptions playgroundOptions, List<Advisor> advisors,
            ChatMemory chatMemory, ChatHistoryService chatHistoryService) {
    ...
    public ChatClientService registerCompleteResponseConsumer(Consumer<ChatHistory> completeResponseConsumer) {
    ...
    private ChatClient buildChatClient(ChatHistory chatHistory) {
        return this.chatClientCache.computeIfAbsent(chatHistory.chatId(), id -> {
            ...
            ChatClient.Builder chatClientBuilder =
                    this.chatClientBuilder.clone().defaultAdvisors(advisors).defaultOptions(chatHistory.chatOptions());
        ...
        return streamWithRaw(chatHistory, prompt, timestamp).map(Generation::getOutput).map(AbstractMessage::getContent)
                .doFinally(signalType -> {
                    if (SignalType.ON_COMPLETE.equals(signalType)) {
                        this.chatHistoryService.updateChatHistory(chatHistory, timestamp);
                        this.completeResponseConsumers.forEach(
                                consumer -> consumer.accept(chatHistory));
                    }
                });
    }

    public Flux<Generation> streamWithRaw(ChatHistory chatHistory, String prompt, long timestamp) {
        return buildChatClient(chatHistory).prompt(
                        new Prompt(new UserMessage(prompt, List.of(), Map.of("chatId", chatHistory.chatId(), "timestamp",
                                timestamp))))
    ...
    public List<Message> getCurrentMessageList(String chatId) {
        return this.chatHistoryService.getMessageList(chatId);
    }
}
